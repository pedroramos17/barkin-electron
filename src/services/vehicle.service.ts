...
const postVehicle = async (vehicle: any) => {
  return axios.post(VEHICLE_ROUTE, vehicle, { headers: authHeader() });
};

const putVehicle = async (id: string, vehicle: any) => {
  return axios.put(`${VEHICLE_ROUTE}${id}`, vehicle, { headers: authHeader() });
};
